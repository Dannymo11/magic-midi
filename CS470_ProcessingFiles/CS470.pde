...
void sendToWekinator() {
    ...
    // Send centroids of poses, hands, and faces
    addCentroidToMessage(poses, nPose, msg);
    addCentroidToMessage(hands, nHand, msg);
    addCentroidToMessage(faces, nFace, msg);

    oscP5.send(msg, new NetAddress("127.0.0.1", 6448));
}

// Helper function to add centroid of detections to the OSC message
void addCentroidToMessage(PtsDetection[] dets, int nDet, OscMessage msg) {
    if (nDet > 0) {
        float avgX = 0, avgY = 0;
        int totalPoints = 0;
        for (int i = 0; i < nDet; i++) {
            for (int j = 0; j < dets[i].points.length; j++) {
                avgX += dets[i].points[j].x;
                avgY += dets[i].points[j].y;
                totalPoints++;
            }
        }
        if (totalPoints > 0) {
            ...
            // No points, send default values
            msg.add(0.0);
            msg.add(0.0);
        }
    } else {
        // No detections, send default values
        msg.add(0.0);
        msg.add(0.0);
    }
}
